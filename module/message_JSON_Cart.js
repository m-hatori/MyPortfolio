...
async function getProductsInCart(user, TIMESTAMP, STATE_NEWORDER, STATE_CHECK_DELIVERYDAY){
  //●前処理
  //商品数確認
  let messagesArray = []
  ...
  let postBackData, postBackDataArray, cartPNum, orderNum, deliveryday, CHECK_ORDER_STATE
  ...
  let card, postCode, buff, stateDeliveryday, STATE_PRODUCTINFO, changeStateDeliveryday = false, changeCart = false, textOrderNum = "", textDeliveryday = "", textDeliverydayState = [false, "商品No."]
  for (let i in cartInfoArray){
    ...
    CHECK_ORDER_STATE = postBackDataArray[postCode_JSON.postBackDataLabels.orderState]
    ...
    //変数取得
    postCode    = postBackDataArray[postCode_JSON.postBackDataLabels.postbackCode]
    cartPNum = Number(i) + 1
    orderNum    = postBackDataArray[postCode_JSON.postBackDataLabels.orderNum].replace("on", "")
    deliveryday = postBackDataArray[postCode_JSON.postBackDataLabels.deliveryday] //yyyy-mm-dd
    sheetId     = postBackDataArray[postCode_JSON.postBackDataLabels.sheetId]
    ...
    //同一商品IDの商品情報を抽出
    pId = postBackDataArray[postCode_JSON.postBackDataLabels.productId]
    console.log(`pId : ${pId}`)
    ...
    //商品情報確認
    STATE_PRODUCTINFO = message_JSON_Order.certificationProductInfo(postBackDataArray, masterProductArray)
    if(STATE_PRODUCTINFO){textOrderNum = "-", textDeliveryday = "-"}
    else{
      //●口数テキストチェック 不要 getCardProductInCart()で後ほどやる
      ...
      const checkTextDeliveryday = message_JSON_Order.chechkTextDeliveryday(deliveryday)
      ...
      //納品日がテキストでなく、納品日チェックが必要
        ...
        if(STATE_CHECK_DELIVERYDAY == 1){
          //TODO: 荷受け日、ブロック日確認 納品終了日が荷受け日のとき、納品期間外になってしまう。。。
          stateDeliveryday = message_JSON_Order.certificationDeliveryday(checkTextDeliveryday[1])//[newDeliveryday, changeStateDeliveryday]
          changeStateDeliveryday = stateDeliveryday[1]

          //変わったらpostBackData書き換え
          if(changeStateDeliveryday){
            user.property.CART[i] = postBackData.replace(deliveryday, stateDeliveryday[0])
            postBackData = user.property.CART[i]
        ...
        //納品期間確認
          ...
          stateDeliveryday = message_JSON_Order.certificationdeliveryPeriod(deliveryday, SD_FMT_LINE, ED_FMT_LINE)//[newDeliveryday, changeStateDeliveryday]
          changeStateDeliveryday = stateDeliveryday[1]
    ...
    */
    ...
    card = getCardProductInCart(postCode, postBackData, postBackDataArray, masterProductArray, SD_FMT_LINE, ED_FMT_LINE, textOrderNum, textDeliveryday, cartPNum, TIMESTAMP, STATE_PRODUCTINFO)
  ...
  let label = "買い物かご内商品一覧", label1 = "発注確定" ,  postdata1
  let label2 = "買い物かご\nリセット", postdata2

  //再発注メッセージ、発注確定ボタンを表示しない。
  //TODO: 再発注メッセージ、発注確定ボタンを表示しない条件決め
  //条件：口数、納品日がテキスト
  let textMessage = ""
  if(textDeliverydayState[0]){
    ...
    //発注確定 全削除ボタン
    postdata2 = postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.allDelete + "--" + TIMESTAMP
    //let label3 = "一括納品日\n変更", postdata3 = postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.setAllDeliveryday + "--" + TIMESTAMP

    //新規発注のみ
    if(STATE_NEWORDER){
      postdata1 = postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.orderConfirm + "--" + TIMESTAMP;
    }
    //再発注伺い
    else{
      postdata1 = postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.reOrderConfirm + "--" + TIMESTAMP;
    }
    ...
    columns.push(flexMessage_ForBuyer.getCardOrderCertification(explainText, label1, postdata1, label2, postdata2))   
  }

  //メッセージ格納
  messagesArray.push(message_JSON.getflexCarouselMessage(label, columns))
...
async function orderCartProductsConfirm(user, TIMESTAMP, orderRecords, reOrderConfirm_STATE){
  //●前処理
  //変数定義
  ...
  for (let i in cartInfoArray){
    ...
    postBackDataArray = postBackData.split("∫")
    sheetId      = postBackDataArray[postCode_JSON.postBackDataLabels.sheetId]
    pId          = Number(postBackDataArray[postCode_JSON.postBackDataLabels.productId])
    CHECK_ORDER_STATE      = postBackDataArray[postCode_JSON.postBackDataLabels.orderState]
    orderNum     = postBackDataArray[postCode_JSON.postBackDataLabels.orderNum].replace("on", "")
    deliveryday  = postBackDataArray[postCode_JSON.postBackDataLabels.deliveryday]//yyyy-mm-dd
      
    //console.log(postBackDataArray)
    console.log(postBackDataArray[postCode_JSON.postBackDataLabels.name])
    ...
    if(!upState || stockNow <= 0 || STATE_PRODUCTINFO || message_JSON_Order.chechkTextDeliveryday(deliveryday)[0]){
    ...
    //再発注ボタン押下時
      ...
      textProdcutsOrderConfirm += 
        "●" + postBackDataArray[postCode_JSON.postBackDataLabels.name] + postBackDataArray[postCode_JSON.postBackDataLabels.norm] +
    ...
    //新規発注ボタン押下時
      ...
      if(CHECK_ORDER_STATE == 0){
        if(certificationCartInfo(cartInfoArray, postBackData, postBackDataArray)){
      ...
      //発注情報 確認済み
        ...
        //const POSTCODEARRAY = postBackDataArray[postCode_JSON.postBackDataLabels.postbackCode].split("-")
        cartInfoArray_doubleOrder.push(
          "●No." + (Number(i) + 1) + "\n" +
          postBackDataArray[postCode_JSON.postBackDataLabels.name] + postBackDataArray[postCode_JSON.postBackDataLabels.norm] +
          "\n希望納品日:" + timeMethod.getDisplayFmtDate(new Date(deliveryday))
...
function certificationCartInfo(cartInfoArray, postBackData, postBackDataArray){
  let checkPostData = postBackData.split(`∫${postBackDataArray[postCode_JSON.postBackDataLabels.CHECK_ORDER_STATE]}∫on`)
      checkPostData = checkPostData[0]
      checkPostData = checkPostData.replace(`${postBackDataArray[postCode_JSON.postBackDataLabels.postbackCode]}∫`, "")
  
  let deliveryday = postBackDataArray[postCode_JSON.postBackDataLabels.deliveryday]
  let one = false, anotherdeliveryday
  for (let buff of cartInfoArray){
    if(buff.includes(checkPostData)){
      
      //商品情報一致、かつ納品日一致    
      anotherdeliveryday = buff.split("∫")[postCode_JSON.postBackDataLabels.deliveryday]

      if(!one){
        if(deliveryday == anotherdeliveryday){
          one = true
        }
      }
      else if(deliveryday == anotherdeliveryday){
        console.log(`---二重発注`)
        return true                  
      }        
    }
...
//●Flex Message 発注内容確認メッセージ 複数発注用（買い物かご）
function getCardProductInCart(postCode, postBackData, postBackDataArray, masterProductArray, SD_FMT_LINE, ED_FMT_LINE, textOrderNum, textDeliveryday, cartPNum, TIMESTAMP, STATE_PRODUCTINFO){
  //postBackdata Tag--PNo → Tag-num-PNo-TIMESTAMP
    let endPostCode = "--" + TIMESTAMP
  ...
  //body
    ...
    if(masterProductArray[property.constPL.columns.stockNow] <= 0 || STATE_PRODUCTINFO) {
      ...
      bodyContents.push(flexMessage_ForBuyer.getCardbodyProductInfo2ForCart(postBackDataArray, masterProductArray[property.constPL.columns.deliveryPeriod]))//商品情報2   商品名～市場納品期間
      ...
      let postdataOrderNum     = postBackData.replace(postCode, postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.orderNum       + endPostCode)
      let postBackDataDeliveryDate = postBackData.replace(postCode, postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.setDeliveryday + endPostCode)
      let headText = "買い物かご商品No." + cartPNum

      footerContents.push(flexMessage_ForBuyer.getCardfooterBottunWidth("口数", headText +"の口数変更", postdataOrderNum, "30%"))
    ...
    let postBackDataDelete = postBackData.replace(postCode, postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.delete + endPostCode)
    footerContents.push(flexMessage_ForBuyer.getCardfooterBottunWidth("削除", "削除", postBackDataDelete, "30%"))
          
  return  flexMessage_ForBuyer.getCardForBuyer(bodyContents, footerContents)
}

//再伺いメッセージ取得
//TODO: 再発注伺いと 納品日がテキストのとき 再発注メッセージ、発注確定ボタンを表示しない。のとどう影響するか
async function getReOrderMessage(textMessage, user, TIMESTAMP, cartInfoArray_doubleOrder, cartInfoArray_doubleCart){
  ...
  let messagesArray = await module.exports.getProductsInCart(user, TIMESTAMP, false, 0)

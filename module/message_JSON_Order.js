//単品発注/複数発注併用 メソッド群
//TODO: ブロック日。。。
//TODO: certification と check の混合解消
...
//●希望口数伺い
async function setOrderNum(TIMESTAMP, postBackData, postBackDataArray, tag, user) {
  ...
  if(stockNow == 0){return message_JSON_irregular.whenStockNone()}
  ...
  let postbackCode
  //単品発注
  if(tag == postCode_JSON.postBackTag.instantOrder){
    //postbackCode
    postbackCode = postCode_JSON.postBackTag.instantOrder + "-" + postCode_JSON.postBackNum.instantOrder.orderNumConfirm + "--" + TIMESTAMP
  }
  //複数発注
  else if(tag == postCode_JSON.postBackTag.cart){
    //postbackCode
    postbackCode = postCode_JSON.postBackTag.cart + "-" + postCode_JSON.postBackNum.cart.setOrderNum + "--" + TIMESTAMP
  }

  postBackData = postBackData.replace(postBackDataArray[postCode_JSON.postBackDataLabels.postbackCode], postbackCode) //PostbackCodeの変更
  postBackData = postBackData + "∫on" //newOrderNumまでの共通postBackData

  for(var i = 0; i <= maxOrderNum; i++){
    if(i == 0){
      items.push({
        "type": "action",
        "action": action_JSON.getPostbackActionWithText("キャンセル", postCode_JSON.getCancelCode(TIMESTAMP), "キャンセル")
      });
    }
    else{
      let textdata = postBackData + i //newOrderNumを追記
      items.push({
        "type": "action",
        "action": action_JSON.getPostbackActionWithText(i, textdata,  i + "口")              
      });
    }
  }
...
function certificationProductInfo(postBackDataArray, masterProductArray){
  //console.log(`照合 postBackData - SS情報`)
  const mData = getTextbyMasterData(masterProductArray)
  const pData = getTextbyPostData(postBackDataArray)
...
function getTextbyPostData(postBackDataArray){
  return postBackDataArray[postCode_JSON.postBackDataLabels.producer].split(" ")[0] +
